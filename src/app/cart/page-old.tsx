...
export default function Cart() {
  ...
  const fetchOldOrders = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `https://dev2024.co.in/web/liquidity-india-backend/admin/api/orderList/${userId}`
      );
      const data = await res.json();
      if (
  ...
  const updateQuantity = async (itemId: string, newQty: number) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://dev2024.co.in/web/liquidity-india-backend/admin/api/updateCartData",
        {
          method: "POST",
          body: formData,
        }
  ...
  const createLiquidityOrder = async (
    ...
    try {
      const res = await fetch(
        "https://dev2024.co.in/web/liquidity-india-backend/admin/api/createOrder",
