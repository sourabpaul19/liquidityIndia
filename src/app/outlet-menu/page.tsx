...
export default function OutletMenu() {
  ...
  useEffect(() => {
  let cancelled = false;
  fetch(`https://dev2024.co.in/web/liquidity-india-backend/admin/api/fetchCategoriesByShop/${shopId}`)
    .then((res) => res.json())
        ...
        const mapped: Category[] = data.categories.map((cat) => ({
          ...
          items: Array.isArray(cat.products)
            ? cat.products.map((p) => ({
                ...
                image: `https://dev2024.co.in/web/liquidity-india-backend/assets/upload/sub_categories/${encodeURIComponent(
                  p.image || ""
                )}`,
